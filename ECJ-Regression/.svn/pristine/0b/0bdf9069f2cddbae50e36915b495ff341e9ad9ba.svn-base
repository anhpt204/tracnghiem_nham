 ...
 */
// LGX + GSC
public class GTCPipeline extends GPBreedingPipeline
    {
    ...
    public GTCPipeline() { parents = new GPIndividual[2]; }
    ...
    public Object clone()
        {
        GTCPipeline c = (GTCPipeline)(super.clone());
    ...
    private void gen_tree_lib_random(GPFunctionSet fs, EvolutionState state, int thread){
    	...
    	//nodes[0] = chon 1 trong nonterminals
    	//nodes[1], nodes[2]: chon 1 trong so cac nodes
    	//node[3..6]: chon 1 trong so cac nodes
    	//node[7-14]: chon 1 trong so cac terminals
    	...
    	{
    		GPNode[] ns = new GPNode[15];
    		for(int i = 0; i < ns.length; i++)
    			ns[i]= nodes[i][random.nextInt(nodes[i].length)].lightClone();
			//GPNode[] ns = {n0, n1, n2, n3, n4, n5, n6};
			GPNode tree = this.gen_tree(ns);
    		double[] tree_sim = problem.getSemantic(state, tree, thread);
    		try{
	...
	private GPNode getTreeFromTreelib(EvolutionState state, int subpopulation, GPIndividual parent, int tree, int threadnum, int numChildren)
	{
        	...
        	else // grab from different sources
        	{
     	    ...
     	    else //GTC
     	    {
     	    	...
     	    	for(int x=0;x<numTries;x++)
     	    	{
     	    		...
     	    		double[] sim1 = problem.getSemantic(state, parents[0].trees[0].child, thread);
     	    		try {            		
                    	List<GPNode> nodes = kd.nearest(sim1, 1);
                    	//int t = (new Random()).nextInt(nodes.size());
                    	subparent1 = nodes.get(0);
     	    		...
     	    		double[] sim2 = problem.getSemantic(state, parents[1].trees[0].child, thread);
     	    		try {            		
                    	List<GPNode> nodes = kd.nearest(sim2, 1);
                    	//int t = (new Random()).nextInt(nodes.size());
                    	subparent2 = nodes.get(0);
                	...
                	//if do not get subparent1 and subparent2, re-select parents
                	if(subparent1 == null || subparent2 == null)
                	{
                    	// grab two individuals from our sources
                 	    if (sources[0]==sources[1])  // grab from the same source
                    		sources[0].produce(2,2,0,subpopulation,parents,state,thread);
