 ...
 * Actually, that's not quite true.  Cozy doesn't have any tree depth restrictions
...
public class RDOPipeline extends GPBreedingPipeline
    {
    ...
    public Object clone()
        {
        RDOPipeline c = (RDOPipeline)(super.clone());
    ...
    public int produce(final int min, 
        ...
        for(int q=start; q < n+start; q++)
            ...
            for(int x=0;x<numTries;x++)
                ...
                double[] desiredSemantics = backPropagation(state, p1, problem.getOutputTraining(), thread);
                if (desiredSemantics == null)
                	continue;
                
                try {            		

                	p2 = state.kdtreelib.nearest(desiredSemantics);
                ...
                //p2 = (GPNode)state.kdtreelib.nearest(desiredSemantics);
