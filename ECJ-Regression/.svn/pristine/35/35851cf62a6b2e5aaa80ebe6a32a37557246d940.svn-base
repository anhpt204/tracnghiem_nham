...
public class SGXEPipeline extends GPBreedingPipeline
    {
    ...
    public SGXEPipeline() { parents = new GPIndividual[2]; }
    ...
    public Object clone()
        {
        SGXEPipeline c = (SGXEPipeline)(super.clone());
    ...
    public int produce(final int min, 
        ...
        for(int q=start;q<n+start; /* no increment */)  // keep on going until we're filled up
            ...
            double TR = (new Random()).nextDouble();
            ...
            child1.trees[0].semanticTraining = new double[parents[0].trees[0].semanticTraining.length];
            child1.trees[0].semanticTesting = new double[parents[0].trees[0].semanticTesting.length];
            
            GPIndividual child2 = null;
            if (n-(q-start)>=2 && !tossSecondParent)
            {
            	child2 = (GPIndividual)(parents[1].lightClone());
            	child2.trees[0].semanticTraining = new double[parents[0].trees[0].semanticTraining.length];
            	child2.trees[0].semanticTesting = new double[parents[1].trees[0].semanticTesting.length];
            }
            
            // geometric semantic crossover on training set
            for(int i = 0; i < parents[0].trees[0].semanticTraining.length; i++)
            {
            	child1.trees[0].semanticTraining[i] = TR * parents[0].trees[0].semanticTraining[i] 
            			+ (1 - TR) * parents[1].trees[0].semanticTraining[i];
            	child1.evaluated = false;
            	
            	if(child2 != null)
            	{
            		child2.trees[0].semanticTraining[i] = (1-TR) * parents[0].trees[0].semanticTraining[i]
            				+ TR * parents[1].trees[0].semanticTraining[i];
            ...
            {
            	child1.trees[0].semanticTesting[i] = TR * parents[0].trees[0].semanticTesting[i]
            			+ (1 - TR) * parents[1].trees[0].semanticTesting[i];
            	
            	if(child2 != null)
            	{
                	child2.trees[0].semanticTesting[i] = (1-TR) * parents[0].trees[0].semanticTesting[i]
                			+ TR * parents[1].trees[0].semanticTesting[i];
            	}
            }
