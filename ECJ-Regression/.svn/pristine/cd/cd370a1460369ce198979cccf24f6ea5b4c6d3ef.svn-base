...
public class HalfBuilder extends KozaBuilder
	...
	@Override
	public GPNode newRootedTreeForTreelib(EvolutionState state,
			GPType treetype, int thread, GPTree gpTree,
			GPFunctionSet functionset, int i, int maxDepth) 
	{
		if (state.random[thread].nextFloat() < pickGrowProbability)
			return growNode(state, 0, state.random[thread].nextInt(maxDepth)+1, treetype, thread, gpTree, i, functionset);
		else
			return fullNode(state, 0, state.random[thread].nextInt(maxDepth)+1, treetype, thread, gpTree, i, functionset);
	}
