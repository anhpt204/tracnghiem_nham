...
public class FloodingLine {
	...
	public List<ArrayList<Coordinate>> getFloodLine(int timestep) {
		...
		for (Integer triIdx : nTri) {
			...
			for (int i = 0; i < 3; i++) {
				...
				for (Integer t : tmp) {
					...
					if (triangleFlooded(tmpTri, time) && (flCommTri++ == 1)) {
						break;
					}
				}
				if (flCommTri < 2) {
	...
	private void init() {
		if (this.fr == null) {
			this.fr = new FloodingReader(this.netcdf);
			this.fr.setReadTriangles(true);
		}
		this.idxMapping = this.fr.getIdxMapping();
		this.triangles = this.fr.getTriangles();
		this.fis = this.fr.getFloodingInfos();

//		for (FloodingInfo fi : this.fis) {
//			fi.getFloodingSeries().clear();
//		}
//		this.fr.resetCache();
//		this.fr = null;
