...
public class SocialCostCalculatorSingleLinkII implements TravelDisutility, BeforeMobsimListener, AfterMobsimListener, AgentDepartureEventHandler, LinkEnterEventHandler, LinkLeaveEventHandler {
	...
	private void computeSocialCosts(LinkInfo li, Id linkId, double congestionEndTime) {
		...
		while (li.agentsLeftLink.size() > 0) {
			...
			// (6Eur/h)
			AgentMoneyEvent am = new AgentMoneyEvent(congestionEndTime, ai.id, money);
