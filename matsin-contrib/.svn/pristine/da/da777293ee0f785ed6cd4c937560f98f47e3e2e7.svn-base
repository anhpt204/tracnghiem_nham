...
public class SocialCostCalculatorSingleLink implements TravelDisutility, IterationStartsListener, AfterMobsimListener, AgentDepartureEventHandler, LinkEnterEventHandler, LinkLeaveEventHandler {
	...
	private void computeSocCost(final LinkInfo info) {
		...
		while (info.agentsLeftLink.size() > 0) {
			...
			double cost = tmp / -600;
			AgentMoneyEvent e = new AgentMoneyEvent(info.lastLeft, ai.id, cost);
