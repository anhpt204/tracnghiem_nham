...
public class SocialCostCalculatorSingleLinkTest extends TestCase {
	...
	public void testSocialCostCalculatorSingleLinkZeroCost() {
		...
		for (Vehicle v : f.agents) {
			v.enterTime = time;
			vehQueue.add(v);
			LinkEnterEvent lee = new LinkEnterEvent(time, v.id, f.l0, null);
			...
			while (vehQueue.size() > 0 && (time - vehQueue.peek().enterTime) >= f.link0.getFreespeedTravelTime(time)) {
				...
				LinkLeaveEvent lle = new LinkLeaveEvent(time, tmp.id, f.l0, null);
		...
		double costs = 0.;
		for (; time >= 0; time--) {
			costs += scalc.getLinkTravelDisutility(f.link0, time, null, null);
		}
	...
	public void testSocialCostCalculatorSingleLinkCostStuckAgent() {
		...
		assertEquals(86330., scalc.getLinkTravelDisutility(f.link0, 60, null, null));
