...
def eaPTA(population, toolbox, cxpb, mutpb, ngen, stats=None,
             halloffame=None, verbose=__debug__):
    """This algorithm reproduce the simplest evolutionary algorithm as
    presented in chapter 7 of [Back2000]_.
    ...
    distinct_ind = {}
    avg_semantic_distance=0
    
    # get distinct individual respect to these semantics in the first generation
    semantics = toolbox.map(toolbox.getSemantic, invalid_ind)
    for semantic in semantics:
        if not distinct_ind.has_key(semantic):
            distinct_ind[semantic] = 1    

    # get average semantic distance between all individuals in the first generation
    poplen = len(population)
    ms = 0
    for i in xrange(poplen):
        for j in xrange(i+1, poplen):
            ms += 1
            avg_semantic_distance = sqrt(sum([(x-y)**2 for x, y in zip(semantics[i], semantics[j])]))
            
    avg_semantic_distance = avg_semantic_distance/ms
    ...
    for gen in range(1, ngen+1):
        ...
        best_fitness_each_gen.append(halloffame[0].fitness.values[0])
    ...
    return population, logbook, best_fitness_each_gen, len(distinct_ind), avg_semantic_distance
