...
MINIMUM_NUMBER_OF_POINTS_IN_CLUSTER = 5
...
#     3. semantic cua 1 tree khong the chi la 1 gia tri duy nhat
    ...
    for line in lines[1:]:
        xs = line.split()
        inputs.append([float(x) for x in xs[:-1]])
            
        # exact output
        output = float(xs[-1])
        
        training_dict[tuple(inputs[-1])] = output
        
    # k-mean clustering
    from sklearn.cluster import KMeans
    
    sample_labels = KMeans(NUMCLUSTERS).fit_predict(inputs)
    
    # get elements in clusters
    all_clusters = []
    ...
    for i in xrange(len(sample_labels)):
        all_clusters[sample_labels[i]].append(inputs[i])
        
    # ignore clusters that number of samples is smaller than threshold
    clusters = [cluster for cluster in all_clusters
                        if len(cluster) > MINIMUM_NUMBER_OF_POINTS_IN_CLUSTER ]
    
#     print len(all_clusters)
#     print len(clusters)
    # randomly choice from clusters for inputs
    trainingInputs = []
    trainingOutputs = []
    
    for cluster in clusters:
        # how many points need to be choice?
        num_points_choice = round(len(cluster) * NUMTRAINSAMPLES / len(inputs))
        
        inps = random.sample(cluster, int(num_points_choice))
        for inp in inps:
            trainingInputs.append(inp)
            trainingOutputs.append(training_dict[tuple(inp)])
        
    N = len(trainingInputs)
