...
//phuong thuc copy node
	node CopyNode(node s){
		...
		if(s != null) 
		{
			...
			//t.semanticTraining = new double[NUMFITCASE];
			//t.semanticTraining = s.semanticTraining.clone();
			//t.semanticTesting = s.semanticTesting.clone();
			/*t.semanticTraining = new double[NUMFITCASE];
			for(int i = 0; i < NUMFITCASE; i++)
				t.semanticTraining[i] = s.semanticTraining[i];*/
			
			/*t.semanticTesting = new double[NUMFITTEST];
			for(int i = 0; i < NUMFITTEST; i++)
				t.semanticTesting[i] = s.semanticTesting[i];*/
			return t;
		}
		return null;
	}
